-- Mark a single match as timed out (48h rule) and keep it out of statistics
...
BEGIN
  ...
  IF v_game.status NOT IN ('waiting'::public.game_status, 'playing'::public.game_status) THEN
    -- Already finished: still make sure a timeout match never lingers in stats
  ...
  SELECT GREATEST(
           v_game.created_at,
           v_game.updated_at,
           COALESCE(MAX(gp.last_active_at), v_game.created_at)
         )
    INTO v_last
...
GRANT EXECUTE ON FUNCTION public.expire_match(uuid) TO anon, authenticated, service_role;

-- Sweep every match that has been idle for more than 48 hours
  ...
  WITH stale AS (
    ...
    WHERE g.status IN ('waiting'::public.game_status, 'playing'::public.game_status)
      AND GREATEST(
            g.created_at,
            g.updated_at,
            COALESCE((SELECT MAX(gp.last_active_at) FROM public.game_players gp WHERE gp.game_id = g.id), g.created_at)
...
GRANT EXECUTE ON FUNCTION public.expire_stale_matches() TO anon, authenticated, service_role;

-- Never record a timed-out match in the statistics
...
BEGIN
  ...
  -- Matches closed by the 48h timeout must never enter the statistics
