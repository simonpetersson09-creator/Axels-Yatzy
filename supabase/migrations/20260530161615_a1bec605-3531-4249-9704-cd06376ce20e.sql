...
DECLARE
  ...
  v_player RECORD;
BEGIN
  SELECT id, status, game_code, forfeited_by INTO v_game
  FROM games
  WHERE id = p_game_id;
  ...
  -- Allow rejoin for waiting/playing AND finished (so client can render results page)
  IF v_game.status NOT IN ('waiting', 'playing', 'finished') THEN
  ...
  RETURN jsonb_build_object(
    ...
    'forfeited_by', v_game.forfeited_by
  );
END;
$$;