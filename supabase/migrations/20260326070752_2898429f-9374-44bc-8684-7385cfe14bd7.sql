...
DECLARE
  ...
  v_has_three boolean;
  v_has_two boolean;
  v_skip_check_scores jsonb;
  v_skip_all_done boolean;
  v_skip_i integer;
BEGIN
  ...
  -- Skip players who have all categories filled (advance to next open player)
  IF NOT v_game_over THEN
    FOR v_skip_i IN 1..v_player_count LOOP
      SELECT CASE WHEN gp.id = v_player.id THEN v_new_scores ELSE gp.scores END
      INTO v_skip_check_scores
      FROM game_players gp
      WHERE gp.game_id = p_game_id AND gp.player_index = v_next_index;

      v_skip_all_done := true;
      FOREACH v_cat IN ARRAY v_valid_cats LOOP
        IF NOT (v_skip_check_scores ? v_cat) OR v_skip_check_scores->>v_cat IS NULL THEN
          v_skip_all_done := false;
          EXIT;
        END IF;
      END LOOP;

      EXIT WHEN NOT v_skip_all_done;
      v_next_index := (v_next_index + 1) % v_player_count;
    END LOOP;
  END IF;
...
BEGIN
  ...
  -- Find first unfilled category and score it as 0
  ...
  -- Skip players who have all categories filled
  IF NOT v_game_over THEN
    FOR v_skip_i IN 1..v_player_count LOOP
      SELECT CASE WHEN gp.id = v_current_player.id THEN v_new_scores ELSE gp.scores END
      INTO v_skip_check_scores
      FROM game_players gp
      WHERE gp.game_id = p_game_id AND gp.player_index = v_next_index;
