...
/* ------------------------------------------------------------------ */
/* Central konfiguration – fyll i riktiga värden inför release          */
/* ------------------------------------------------------------------ */

/** Googles officiella test-ID (används under utveckling). */
...
export type ShowAdResult = 'shown' | 'unavailable' | 'failed';
...
export async function showOptionalInterstitial(): Promise<ShowAdResult> {
  ...
  try {
    ...
    preparedAt = 0; // förbrukad – måste laddas om inför nästa tryck
    return 'shown';
  } catch {
    preparedAt = 0;
    return 'failed';
